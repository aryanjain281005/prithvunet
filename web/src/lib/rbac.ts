...
const ROUTE_RULES: Array<{ pattern: RegExp; roles: UserRole[] }> = [
  ...
  { pattern: /^\/admin\/complaints(?:\/|$)/, roles: ["super_admin", "regional_officer"] },
  { pattern: /^\/admin\/copilot-logs(?:\/|$)/, roles: ["super_admin", "regional_officer"] },
...
export function canRoleAccessApi(role: UserRole, apiPath: string): boolean {
  ...
  if (
    apiPath.startsWith("/api/admin/complaints") ||
    apiPath.startsWith("/api/admin/copilot-logs") ||
    apiPath.startsWith("/api/admin/persistence/validate")
  ) {
    return role === "super_admin" || role === "regional_officer";
  }
