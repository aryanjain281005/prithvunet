...
export default function CitizenPage() {
  ...
  const [complaintSuccess, setComplaintSuccess] = useState<{ complaintId: string; createdAt: string } | null>(null);
  ...
  const submitComplaint = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload = (await response.json()) as { complaintId?: string; createdAt?: string; error?: string };
      if (!response.ok || !payload.complaintId || !payload.createdAt) {
        setComplaintError(payload.error || "Complaint submission failed");
        return;
      }

      setComplaintSuccess({ complaintId: payload.complaintId, createdAt: payload.createdAt });
      setComplaint({
        name: "",
        mobile: "",
        email: "",
